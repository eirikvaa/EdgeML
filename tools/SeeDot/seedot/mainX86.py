    ...
    for item in os.listdir(src):
        ...
        if os.path.exists(destination_path):
            if os.path.isdir(destination_path):
                # os.remove() cannot remove directories with files, so we're using shutil.rmtree instead
...
class MainX86:
    ...
    def setup(self):
        ...
        elif self.target == Common.Target.X86:
            pass
    ...
    def convert(self, version, datasetType, target):
        ...
        if target == Common.Target.Arduino:
            ...
            if version == Common.Version.Fixed and datasetType == Common.DatasetType.Testing:
                outputDir = os.path.join(Common.outdir, "input")
                datasetOutputDir = outputDir
        else:
            assert False
    ...
    def compileForTarget(self):
        ...
        destinationFileName = "model.h" if self.target == Common.Target.Arduino else "seedot_fixed_model.h"
        destFile = os.path.join(Common.outdir, destinationFileName)
        shutil.copyfile(srcFile, destFile)
    ...
    def runForFixed(self):
        ...
        fileNames = [
            'seedot_fixed.cpp', 'protonn_float.cpp', 'protonn_float_model.h',
            'profile.h', 'profile.cpp', 'predictors.h', 'main.cpp',
            'library.h', 'library.cpp', 'datatypes.h', 'bonsai_float.cpp',
            'bonsai_float_model.h'
        ]

        for fileName in fileNames:
            srcFile = os.path.join(Common.tempdir, fileName)
            destFile = os.path.join(Common.outdir, fileName)
            shutil.copyfile(srcFile, destFile)
...
class MainX86Driver:
    ...
    def run(self):
        ...
        obj = MainX86(algo, Common.Version.Fixed, Common.Target.X86,
